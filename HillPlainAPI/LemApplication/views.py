...
def postThreadLemSheet(inputData):
    try:
        ...
        for key, value in serializer.errors.items():
            # logger.info the key and each error code and message
            logger.error(dumps({'Error Key': key, 'Error Value': value}, indent = 4))
...
async def lemSheet(request:ASGIRequest):
    ...
    try: 
        ...
        logger.debug('date ====================================== %s'%request.body)
    ...
    try: 
        ...
        if request.method == 'POST':
            ...
            if serializer.is_valid():
                ...
                url ='http://4.149.229.109:8000//HpClockifyApi/rateSheet'
