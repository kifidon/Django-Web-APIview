...
from HillPlainAPI.settings import AZURE_STORAGE_CONNECTION_STRING, AZURE_CONTAINER_NAME
"""
...
def TimeStatusEvent(request, start_date=None, end_date= None):
    logger = setup_server_logger()
    logger.info(f'Time Status Report Called')
...
def ViewServerLog(request):
   ...
   # Get Azure Blob Storage connection details from environment variables
   ...
   try:
      ...
      if not blob_client.exists():
         return HttpResponse('Logger file not found in Azure Blob Storage', status=404)

      # Download the blob's content
      blob_data = blob_client.download_blob().readall()
      log_contents = blob_data.decode('utf-8')  # Decode bytes to string

      # Extract the last 5000 lines and reverse their order
      lines = log_contents.splitlines()[-5000:]
      reversed_lines = '\n'.join(reversed(lines))

      return HttpResponse(reversed_lines, content_type='application/json')

   except Exception as e:
      # Handle errors (e.g., connection issues, permission errors, etc.)
      return HttpResponse(f"An error occurred: {str(e)}  ({e.__traceback__.tb_lineno})", status=500)
...
def ViewTaskLog(request):
   ...
   logger.info("REFRESHING TASK LOGS")
   # Get Azure Blob Storage connection details from environment variables
   connection_string =AZURE_STORAGE_CONNECTION_STRING
   container_name = AZURE_CONTAINER_NAME
...
"""
@api_view(['GET'])
def BillableNonBillable(request:ASGIRequest, start_date = None, end_date = None):
   logger = setup_server_logger()
...
@api_view(['GET'])
def TimeOffReport(request, startDate= None, endDate = None):
   logger = setup_server_logger()
   logger.info(f'Billable vs. Non billable Report Called')
   folder_path = TimeOffReportCaller(startDate, endDate )
   return download_text_file(folder_path)