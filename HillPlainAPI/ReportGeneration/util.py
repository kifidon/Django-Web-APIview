...
def TimeStatusCaller(start_date=None, end_date=None):
    ...
    if start_date is None or end_date is None:
        ...
        start_date = (datetime.strptime(start_date, '%Y-%m-%d') - timedelta(days=6 - datetime.strptime(start_date, '%Y-%m-%d').weekday())).strftime('%Y-%m-%d')    
        
    file_path = TimeStatusGenerator(start_date, end_date)
    logger.info(f"{file_path}")
    return file_path
    
def TimeOffReportCaller(start = None, end = None):
    # Set default date range to last complete week if no dates provided
    if (start == None or end == None):
        start = datetime(datetime.now().year, 1,1).strftime('%Y-%m-%d')
        end = datetime(datetime.now().year, 12,31).strftime('%Y-%m-%d')
    file_path = TimeOffGenerator(start, end)
