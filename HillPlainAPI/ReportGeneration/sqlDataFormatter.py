...
def fit_to_width(ws, max_width):
    """
    Adjust the scale of the worksheet to fit within the specified max width (number of columns).
    """
    column_count = ws.max_column
    scale = min(100, int(max_width / column_count * 100))
    ws.page_setup.scale = scale

def add_headers_and_footers(input_file):
    wb = load_workbook(input_file)
    for sheet in wb.sheetnames:
        ws = wb[sheet]

        # Add headers and footers
        fit_to_width(ws, max_width=25)  # Fit to 10 columns wide
        ws.oddFooter.center.text = "Page &P of &N"
        ws.oddFooter.left.text = "Date: &D"
        ws.oddFooter.right.text = "Time: &T"
...
def convertXlsxPdf(folder_path, file_path, retry=0):
    try:
        ...
        add_headers_and_footers(file_path)
        # Command to convert .xlsx to PDF using LibreOffice
        result = subprocess.run(
            [
                "libreoffice",  # Command to call LibreOffice
                "--headless",  # Run without GUI (headless mode)
                "--convert-to", "pdf",  # Convert to PDF format
                file_path,  # Path to the input file
                "--outdir", folder_path  # Output directory for the PDF
            ],
        ...
        pdf_file = os.path.join(folder_path, f"{os.path.splitext(os.path.basename(file_path))[0]}.pdf")
...
def GenerateBilling(file_path, pId, startDate, endDate, logger, month, year):
    ...
    #Remove Nulls 
        ...
        with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
            ...
            title_format = workbook.add_format({'align': 'center', 'valign': 'vcenter', 'bold': True, 'bg_color': '#FCD5B4', 'font_size': 24, 'border': 2})
            worksheet.merge_range(0,0,1,11, "Hill Plain - Monthly LEM (Indirects)", title_format)
            #subTotals Format
            ...
            headers_format = workbook.add_format({'align': 'center', 'bold': True, 'bg_color': '#F2F2F2', 'font_size': 16, 'border': 1})
            ...
            row += 1 
            #insert image 
            image_path = os.path.abspath(r"static/drawables/hpLogo.png")  # Convert to absolute path
        ...
        # Table  Data 
            worksheet.merge_range(row,0,row,11, 'LABOUR', headers_format)
            row += 1
        ...
        # equipment Table 
            if equipmentTotal > 0:
                row += 1
                worksheet.merge_range(row,0,row,11, 'EQUIPMENT', headers_format)
                row += 1
        ...
        # Description of work data 
            ...
            for description in descriptionData:
                if description[0] != previousEmp:
                    ...
                    worksheet.merge_range(row,0,row,11, description[0], headers_format)
                    row +=1
...
def TimeStatusGenerator(start = None, end = None):
    ...
    try: 
        ...
        while datetime.strptime(start, '%Y-%m-%d')  <= datetime.strptime(endRange, '%Y-%m-%d'):
            ...
            query = f'''
                with approvedTimesheet as (
                    select eu.id from Timesheet ts 
                    inner join EmployeeUser eu on eu.id = ts.emp_id
                    where 
                        ts.status = 'APPROVED' 
                ...
                noTimesheet as (
                    select eu.id from AttendanceApproved ap
                    inner join EmployeeUser eu on eu.name = ap.name
                    where 
                        ap.name = eu.name 
                ...
                Select Distinct 
                    ...
                    eu.manager,
                    Case 
                        when at.id is not Null then 1
                        when pt.id is not null then 2
                        when nt.id is not null then 3
                        when tt.id is not null then 4
                        else 5
                    end as [Status]
                From EmployeeUser eu
                left join approvedTimesheet at on at.id = eu.id
                left join pendingTimesheet pt on pt.id = eu.id 
                left join noTimesheet nt on nt.id = eu.id 
                left join timeOffTimesheet tt on tt.id = eu.id  
...
def TimeOffGenerator(start, end):
    cursor, conn = sqlConnect()
    logger = setup_background_logger()
    query = '''
        with cteBanked as (
            Select bdd.empID, bdd.empName, bdd.accuredOn, bdd.total, bdd.BankedHours, bdd.Reason from BankedHRSDetailed bdd

            UNION

            Select bdd.empID, bdd.empName, Null, Null, sum(bdd.BankedHours), Null from BankedHRSDetailed bdd
            group by bdd.empName, bdd.empID
        )
        select 
            cb.empID as [User ID],
            cb.empName as [Name],
            cb.accuredOn as [Accrual Date],
            cb.total as [Worked Hours],
            cb.BankedHours as [Accrued],
            cb.Reason as [Description]
        From cteBanked cb
        order by cb.empName, cb.accuredOn
    '''
    logger.debug(query)
    cursor.execute(query)
    rows = cursor.fetchall()
    rows = [[None if val is None else val for val in rowData] for rowData in rows]
    rowHeaders = [desc[0] for desc in cursor.description]
    # Generate Folder for spreadsheets
    current_dir = BASE_DIR
    reports = 'Reports'
    folderName = 'TimeOff'
    internalFolder = datetime.now().strftime("Generated_%Y-%m-%d")
    folder_path = os.path.join(current_dir,reports,folderName, internalFolder)
    logger.debug(f'Created Folder at {folder_path}')
    if not os.path.exists(folder_path):
        os.makedirs(folder_path )
    file_path = os.path.join(folder_path, f"{start} to {end}.xlsx")

    with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
        #Generate file and initilize writers and formats 
        workbook = writer.book
        worksheet = workbook.add_worksheet("Hill Plain - LEM")
        writer.sheets['Hill Plain - TimeOff'] = worksheet 
        ...
        titleFormat.set_bg_color('#D9D9D9')
        #Notes Format
        notesFormat = workbook.add_format({'bold': True, 'align': 'center', 'valign': 'vcenter','text_wrap':True, 'italic':True})
        notesFormat.set_font_size(8)
        #file Heaaders
        headerFormat = workbook.add_format({'bold': True, "italic": True, "align": 'right', 'valign': 'vcenter'})
        headerValueFormat = workbook.add_format({'italic': True,'align': 'left', 'text_wrap': True, 'valign':'vcenter',  'num_format': 'yyyy-mm-dd'})
        #columnNameFormat 
        columnNameFormat = workbook.add_format({'bold': True, 'num_format': 'yyyy-mm-dd', 'align': 'center', 'valign': 'vcenter'})
        columnNameFormat.set_border(1)
        columnNameFormat.set_bg_color('#808080')
        columnNameFormat.set_font_color('#ffffff')
        # Text Data Format 
        ...
        highlightFormat = workbook.add_format({"bold": True, 'italic': True, 'align': 'center'})
        highlightFormat.set_border(1)
        highlightFormat.set_bg_color('#92CDDC')
        # date Data Format 
        dateFormat = workbook.add_format({'num_format': 'yyyy-mm-dd', "align": 'center', 'valign': 'vcenter'})
        dateFormat.set_border(1)
        #totals
        ...
        worksheet.merge_range(row,0,row+1,13 , f'Salary Time Off Requests', titleFormat)
        row += 2
        headers = {
                'Generated on:': datetime.now().strftime('%Y-%m-%d at %H:%M'),
                'Date Range Start:': start,
        ...
        for key, value in headers.items():
            ...
            row += 2
        notes = '''
NOTES: Banked Time Off Policy detailed overiview. This report contains the data used for calculations to accrue and track each employees Banked time. the "BANKED-OFFICE" Tag and "BANKED - SITE" Tag in Clockify notify the system to bank all time over 8 and 8.5 hours respectively. Time is banked under the following circumstances.
- Over 8(.50) hours worked on a weekday 
- All time worked on a weekend with a banked tag
- All Time Worked on a holiday with a banked tag
'''
        worksheet.merge_range(2,5, 7,11, notes,notesFormat)
        logger.debug(row)
        row += 1
        logger.debug(row)
        image_path = os.path.abspath(r"static/drawables/hpLogo.png")  # Convert to absolute path
        ...
        try:
            for data in rows:
                ...
                if currentEmp != previousEmp:
                    #Emp Summary 
                    if totalData is not None:
                        index = 1
                        for index in range(1,len(totalData)):
                            if index !=5:
                                worksheet.merge_range(row,column,row,column+1, totalData[index],totalFormat)
                                column += 2
                            else:
                                worksheet.merge_range(row,column,row,column+wide, totalData[index],totalFormat)
                                column += wide +1
                        row += 1
                        column = 0
                        query = '''
                            Select 
                                bp.[All Time Banked Hours], 
                                bp.[All Time Used Banked Hours], 
                                bp.balance as [Balance]
                            From BankedTimeOffPolicy bp
                            where bp.id = ? 
                        '''
                        cursor.execute(query, totalData[0])
                        policyRow = cursor.fetchone()
                        policyHeaders =  [desc[0] for desc in cursor.description]
                        for i, policyData in enumerate(policyRow):
                            worksheet.merge_range(row, 0, row, 2, policyHeaders[i], highlightFormat)
                            worksheet.write(row,3, round(float(policyData), 2),highlightFormat)
                            row += 1
                        row +=1    

                    previousEmp = data[1] #emp Name
                    index=1

                    #column Header 
                    for index in range(1, len(rowHeaders)):
                        if index != 5:
                            worksheet.merge_range(row, column, row+1, column+1, rowHeaders[index],columnNameFormat)
                            column +=2
                        else:
                            worksheet.merge_range(row,column,row+1,column +wide, rowHeaders[index],columnNameFormat)
                            column += wide +1
                    row += 2
                    totalData = data
                    continue
                ...
                for index in range(1,len(data)):
                    if index in (1,3,4):
                        worksheet.merge_range(row, column, row, column+1, data[index], textFormat)
                        column += 2
                    elif index == 2: 
                        worksheet.merge_range(row, column, row, column+1, data[index], dateFormat)
                        column += 2
                    else:
                        worksheet.merge_range(row, column, row, column+wide, data[index],textFormat)
                        column+= wide +1
                row += 1
                previousEmp = data[1] 
