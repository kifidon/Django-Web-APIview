...
class Timeoffpolicies(models.Model):
    id = models.CharField(primary_key=True, max_length=50)  # The composite primary key (id, wID) found, that is not supported. The first column is selected.
...
class TimeOffRequests(models.Model):
    ...
    workspaceId = models.ForeignKey(Workspace, models.DO_NOTHING, db_column='workspace_id')
    createdAt = models.DateTimeField(blank=False, null= False, db_column='createdAt')
