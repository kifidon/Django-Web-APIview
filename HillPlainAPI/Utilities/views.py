...
from HillPlainAPI.Loggers import setup_server_logger, setup_background_logger
...
class  AsyncViews(View):
    def __init__(self, **kwargs):
        ...
        self._logger = setup_server_logger()
        self._taskLongger = setup_background_logger()
        self._secret = {}  
        self._action = 'CU'

    @classmethod
    def as_view(cls, **initkwargs):
        # Make the as_view method asynchronous and explicitly await the dispatch method
        view = super().as_view(**initkwargs)

        # Ensure that the dispatch method is awaited correctly by calling it asynchronously
    ...
    async def dispatch(self, request, *args, **kwargs):
        ...
        try:
            ...
            self._logger.warning('Unknown Exception, attempting to handle')
...
"""
def count_working_daysV2(start_date:datetime | str, end_date: datetime, excludeDays=[] ):
    if isinstance(start_date, str):
        # strip the Time and zone information from the string 
...
'''
def toMST(date_obj: datetime |str, time = False):
    ...
    try: 
        ...
        raise Exception(f"{e.__traceback__.tb_lineno} - {type(e)} - {str(e)}")
'''
...
def sqlConnect():
    ...
    while True: 
        if (retry > 3):
            logger.critical("Cannot open connection to database")
