...
from Clockify.models import Timeoffpolicies
...
def updateSalaryVacation(userId, cursor):
    policy = Timeoffpolicies.objects.get(id = '67865ceb975d1e5017b88d02')
    
    query = f'''
    select * from SalaryPolicyAccrual sp
        where sp.id = ?
    '''
    ...
    try:
        ...
        url = f"https://api.clockify.me/api/v1/workspaces/65c249bfedeea53ae19d7dad/time-off/balance/user/{userId}"
        headers = {
            'X-Api-Key': ClockifyPullV3.getApiKey()
        }
        response = ClockifyPullV3.requests.get(url, headers=headers)
        if response.status_code == 200:
            ...
            for balanceInternal in policies['balances']:
                if policy.id == balanceInternal['policyId']:
                    clockifyBalance = round(balance[0] - balanceInternal["balance"],2)
                    break
                else:
                    continue
        if clockifyBalance == 0:
            return  0
        headers = {
                'X-Api-Key': 'YWUzMTBiZTYtNjUzNi00MzJmLWFjNmUtYmZlMjM1Y2U5MDY3',
                'Content-Type': 'application/json'
            }
        url = f'https://pto.api.clockify.me/v1/workspaces/65c249bfedeea53ae19d7dad/balance/policy/{policy.id}'

        payload = {
            "note": "Setting Accured vacation time for this date",
            "userIds": [
                userId
            ],
            "value": clockifyBalance
        }
        logger.info(reverseForOutput(payload))
        result = patch(url=url, headers= headers, json=payload)
        if result.status_code == 204:
            return clockifyBalance
        else: 
